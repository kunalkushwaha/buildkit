...
func capListContains(capList []string, capability osdefs.Capability) bool {
...
func isSyscallWithArgsBlockedBySeccomp(seccompProfile specs.LinuxSeccomp, syscallName osdefs.Syscall, syscallArgs []specs.LinuxSeccompArg) bool {
...
func AreSyscallsWithArgsBlockedBySeccomp(seccompProfile specs.LinuxSeccomp, syscallsWithArgs map[osdefs.Syscall][]specs.LinuxSeccompArg) bool {
...
func AreSyscallsBlockedBySeccomp(seccompProfile specs.LinuxSeccomp, syscallNames []osdefs.Syscall) bool {
...
func isSyscallWithArgsAllowedBySeccomp(seccompProfile specs.LinuxSeccomp, syscallName osdefs.Syscall, syscallArgs []specs.LinuxSeccompArg) bool {
...
func AreSeccompSyscallsWithArgsAllowed(seccompProfile specs.LinuxSeccomp, syscallsWithArgs map[osdefs.Syscall][]specs.LinuxSeccompArg) bool {
	for syscallName, syscallArgs := range syscallsWithArgs {
...
func AreSyscallsAllowedBySeccomp(seccompProfile specs.LinuxSeccomp, syscallNames []osdefs.Syscall) bool {
	for _, syscallName := range syscallNames {
...
func isCapBlocked(linuxCaps specs.LinuxCapabilities, capability osdefs.Capability) bool {
...
func AreCapsBlocked(linuxCaps specs.LinuxCapabilities, capabilities []osdefs.Capability) bool {
...
func isCapAllowed(linuxCaps specs.LinuxCapabilities, capability osdefs.Capability) bool {
	return capListContains(linuxCaps.Bounding, capability) && capListContains(linuxCaps.Permitted, capability) &&
		capListContains(linuxCaps.Inheritable, capability) && capListContains(linuxCaps.Effective, capability)
}

// AreCapsAllowed checks that capabilities in the provided cap list are allowed
func AreCapsAllowed(linuxCaps specs.LinuxCapabilities, capabilities []osdefs.Capability) bool {
	for _, capability := range capabilities {
...
func capsListMatchRefSet(refWithConstraints map[osdefs.Capability]bool, capList []string) bool {
...
func capsListMatchRefWithConstraints(capList []string, capsToAdd, capsToRemove []osdefs.Capability) bool {
...
func OCICapsMatchRefWithConstraints(capabilities specs.LinuxCapabilities, capsToAdd, capsToRemove []osdefs.Capability) bool {
	capStrLists := [][]string{
